...
const ArtifactCreatePage = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    alert("Artifact created! (not implemented)");
  };
    ...
    <div className="p-8 bg-white min-h-screen text-gray-900">
      ...
      <form onSubmit={handleSubmit} className="flex gap-8 max-w-4xl">
        ...
        <div className="flex-1 bg-white border border-gray-200 rounded-lg p-6 shadow">
          <h3 className="font-semibold text-lg mb-2">Artifact Details</h3>
          <label className="block font-semibold mb-1 mt-2">Name</label>
          <input className="w-full rounded px-3 py-2 bg-gray-100 text-gray-900" value={name} onChange={e => setName(e.target.value)} />
          <label className="block font-semibold mb-1 mt-2">Origin</label>
          <input className="w-full rounded px-3 py-2 bg-gray-100 text-gray-900" value={origin} onChange={e => setOrigin(e.target.value)} />
          <label className="block font-semibold mb-1 mt-2">Period</label>
          <input className="w-full rounded px-3 py-2 bg-gray-100 text-gray-900" value={period} onChange={e => setPeriod(e.target.value)} />
        ...
        <div className="flex-1 bg-white border border-gray-200 rounded-lg p-6 shadow flex flex-col justify-between">
          <h3 className="font-semibold text-lg mb-2">Artifact Image</h3>
          ...
          <div className="flex gap-2 mt-4 justify-end">
            ...
            <button type="submit" className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2 rounded font-semibold shadow">Create Artifact</button>
          </div>
        </div>
      </form>
    </div>
